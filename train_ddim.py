...
flags.DEFINE_float("DDIM_weight_decay", 1e-4, "The autoencoders weight decay")
...
#! Others
flags.DEFINE_integer("DDIM_batch_size", 64, "The DDIMs batch size")
flags.DEFINE_integer("DDIM_run_seed", 0, "The seed used to generate JAX prng")
flags.DEFINE_list("DDIM_batch_dims", [64, 64], "The dimensions of a latent space tensor")
...
#! Model parameter
flags.DEFINE_list("DDIM_feature_sizes", [96, 128, 160], "The sizes for the unet as a list of features sizes of length n. The unet will have n-1 layers, and the final feature is a resnet block at the bottom")
...
def train_step(state, batch, rng):
    ...
    def compute_loss(params):
        ...
        loss = losses.L1(orig_batch, pred_batch).mean()
...
def train() -> TrainState:
    ...
    print(dataset.shape) #e.g. 288, 64, 64, 64 = (N, B), (LSD1, LSD2)
    ...
    for epoch in range(FLAGS.DDIM_epochs):
        pbar = tqdm(range(len(dataset)), desc=f'Epoch {epoch}')
        for i in pbar:
...
def evaluate(batch, ddim_state, ae_state, rng):
    ...
    generated_ecg = ae_state.apply_fn(ae_variables, generated_batch, method= AutoEncoder.decode)
