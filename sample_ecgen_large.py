...
from train_ddim_large import TrainState as DDIMTrainState
...
def get_ddim(rng: jax.random.PRNGKey) -> DDIMTrainState:
    config = {
        "DDIM_convolution_filters": [128, 128, 128, 128, 128, 128, 128],
        "DDIM_batch_dims": [5120, 16],  # shape of each sample
        "DDIM_block_depths": 2,
        "DDIM_learning_rate": 0.0001,
        "DDIM_weight_decay": 0.001,
        "DDIM_ema_momentum": 0.99,
        "checkpoint_dir": "/home/dominik.kranz/data/checkpoints/ecgen/large/290"
    }
    ...
    ddim_state = flax.training.checkpoints.restore_checkpoint(
        ckpt_dir=checkpoint_path, target=ddim_state, step=290
    )
    return ddim_state
...
def generate_ddim(ddim_params, rng, sample_count, ddim_state):
    """
    Generate a batch of shapes (sample_count, 5120, 16) using the DiffusionModel.
    """
...
def autoencode(ae_params, x, ae_state):
    ...
    return ae_state.apply_fn({"params": ae_params}, embedded, method=AutoEncoder.decode)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--config", type=str, default="config/inference_config.yml", 
    ...
    df.to_parquet("/home/dominik.kranz/data/generated_ecgen_large_deeper_unet_bd_2_200k_ckpt_290.parquet")
